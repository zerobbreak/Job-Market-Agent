...
from app.core.dependencies import CurrentUser, ProfileServiceDep, get_cv_service
...
async def analyze_cv(
    ...
    """Upload and analyze a CV file — parse, extract, and run AI analysis."""
    # Basic validation (could move to service or middleware but keeping simple for now)
    if not cv_file.filename:
        raise BadRequestError("No file provided")

    try:
        content = await cv_file.read()
        # In a real app, we'd pass the file content to the service
        # For now, we'll keep the temp file logic inside service if needed
        # but let's assume we implement this properly in ProfileService later
        
        # Placeholder for actual analysis logic which is currently in pipeline_service
        from services.pipeline_service import JobApplicationPipeline
        import os
        from app.core.config import get_settings
        settings = get_settings()
        
        temp_path = os.path.join(settings.upload_folder, f"{user.id}_{cv_file.filename}")
...
async def regenerate_cv(
    ...
    user: CurrentUser,
    cv_service: Annotated["CVService", Depends(get_cv_service)], # Avoid circular if needed
):
    """Regenerate and optimize the CV using AI with chosen template."""
    # This will use the CVService to handle the logic
    # Placeholder for now
